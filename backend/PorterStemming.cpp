...
int cons(char a[], int i) {
    switch (a[i]) {
        ...
        case 'u':
            return 0;
        case 'y': {
            if (i == k0) return 1; else !cons(a, i - 1);
        }
        default:
            return 1;
    }
...
int m(char a[], int j, int k) {
    ...
    while (1) {
        ...
        if ((c == v) && (r <= k + 1)) {
            ...
            value = value + 1;
            c = 0;
            v = 0; /*counts the number of VC sequences*/
        }
        if (r > k) break;
    }
...
int cvc(int i, char a[], int j) {
    ...
    if (i <= k0 + 2) {
        if (!cons(a, i) || cons(a, i - 1) || !cons(a, i - 2)) {
            int ch = a[i];
            if (ch == 'w' || ch == 'x' || ch == 'y')
                return 0; /* In CVC, the last C should not be w,x,y as per the algo.*/
        }
    }
    return 1;
}
...
void setto(int len_inc, char *add, char a[]) {
    ...
    if (parameter == 1) {
        ...
        memset(a, '\0',
               sizeof(a)); /*setting a to null. Basically to remove out the junk values when used later in the func*/
...
void step1ab(char a[], int j) { //cout<<"Enter 1AB";
    int last_w = j;
    int h = 0;
    if (a[k] == 's') {
        if (ends(a, "sses", 4, last_w) == 1) { k -= 2; }
    ...
    } else if (ends(a, "eed", 3, last_w) == 1) { if (m(a, last_w, k) > 1)k--; }
...
void step5(char a[], int j) {
    ...
    if (a[k] == 'e') {
        int x = m(a, j, k);
        if (x > 1) { k--; } else if ((x == 1) && !cvc(k - 1, a, 0)) k--;
    }
    if (m(a, j, k) > 1) {
        if (doublec(a, k)) {
            if (a[k] == 'l') k--;
